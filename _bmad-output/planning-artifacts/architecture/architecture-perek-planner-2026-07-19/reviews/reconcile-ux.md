...
- `_bmad-output/planning-artifacts/ux-designs/ux-perek-planner-2026-07-19/.memlog.md` (skim — decisions that shaped final UX spines)
- `_bmad-output/planning-artifacts/architecture/architecture-perek-planner-2026-07-19/ARCHITECTURE-SPINE.md`
- `_bmad-output/planning-artifacts/architecture/architecture-perek-planner-2026-07-19/.memlog.md` (skim — coaching decisions vs distilled spine)
...
|---|---|---|---|
| **Pantry/staples on Shopping list by default** — no per-item opt-in prompt; user filters at Perekrestok | EXPERIENCE Foundation, `shopping-list-cta`, Voice | No mention of Pantry items in list composition | Directly affects shopping-list generator and FR-19/FR-12 interpretation. Overrides earlier memlog "prompt each time" — final UX is **include by default**. |
...
| **Settings + `store-picker`** — concrete Perekrestok store; **set once**, not before each Menu | EXPERIENCE IA, `store-picker` | Store entity in ER; **no user store preference** table or Settings route |
...
- Final UX (memlog override 2026-07-19): **staples included on Shopping list by default**, no per-item prompt; filtering at Perekrestok.
