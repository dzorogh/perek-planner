...
| store-catalog-api | 0.2.2 | PyPI latest **0.2.2**; `requires_python >=3.10` | ✅ Confirmed | Unofficial scraper client — operational risk, not version risk |
...
- GitHub: Open-Inflation/primary_api — explicitly **not official**; mimics site traffic.
...
| the store site APIs (unofficial) | Via store-catalog-api | Yes with adapter boundary | ✅ (with operational caveat) |
...
- **Explicitly researched:** `store-catalog-api` PyPI 0.2.2, unofficial nature, Dokploy hosting, OpenRouter, Supabase as BaaS.
...
7. **store-catalog-api operational risk** — Version OK; unofficial scraper dependency should stay in Deferred/Risk, not just memlog.
...
- PyPI: https://pypi.org/pypi/store-catalog-api/json
...
| Technologies fit architecture | **Yes** (with store-catalog-api operational caveat) |
