...
- Personal web batch-cooking planner: pick **1–4 days** → one **Menu**, one **Perekrestok order**, one **cook session**; default **3 meals/day × 2 people**; purchase **outside** the app via always-**copyable Shopping list** (+ optional store link). *(extract-prd §Product one-liner; memlog UJ-1 confirmed)*
- Single operator **Sergey**; unauthenticated users cannot access Menus or history. *(FR-23; memlog stakes + UJ-1)*
- **UJ-1** end-to-end: sign-in → pick days → **Menu slot edit** → **Portion plan** → **Shopping list** → external Perekrestok. *(memlog: “UJ-1 flow confirmed OK as stated”)*
...
- **Concrete Perekrestok store** selection; v1 default context **д. Алабино, 92**. *(FR-16; memlog: “Store selection surface in v1: yes”)*
...
6. **Store selection (FR-16)**  
   - *PRD stated; UX confirmed:* `(decision) Store selection surface in v1: yes (concrete Perekrestok store; default Алабино 92)` — no conflict; explicit UX commitment.
