# Google Stitch — perek-planner UX handoff

Paste into https://stitch.withgoogle.com. Produce: visual directions + key-screen HTML (and DESIGN.md if the tool emits it). Save all outputs into the project folder you were given (`ux-perek-planner-2026-07-19/`), preferably under `imports/` or `.working/`.

## Product

**perek-planner** — personal web batch-cooking planner for one operator (Sergey).  
Open app → pick **1–4 days** → get a **Menu** of **Recipes** for **one Perekrestok order** and **one cook session**, with a **Portion plan** by day and meal (default: 3 meals/day × **2 people**). Purchase happens **outside** the app via a always-**copyable Shopping list** (optional Perekrestok store link when available).

Default store context: **Perekrestok, д. Алабино, 92**.
...
Opens the web app on desktop → picks Menu length (1–4 days) → receives suggested Recipes for breakfast/lunch/dinner + optional no-cook Snacks → **edits slots** (swap/refuse/empty allowed) → reviews Portion plan → copies Shopping list (and optionally opens store link) → leaves to buy on Perekrestok site.
...
8. **Store selection** — pick a concrete Perekrestok store (not free-text address).
