...
- Plans must **not collapse** when the store is missing a key ingredient.
- **Finish the purchase outside this app** — copyable shopping list is always enough; the store link when a working link is available.
...
- **External:** store (hand off list and/or link — no checkout inside keplo).
- Single store in first version, address **д. Алабино, 92**.
...
4. **Validate against store** — only recipes whose critical ingredients map to the store products, fridge storage ≥ window, availability “enough” on last check; prefer cheaper analogs without ultra-cheap quality collapse.
5. **Build one combined shopping list** — pantry items gate eligibility but are **opt-in for cart** (A6); always copyable; the store link when possible.
...
10. **Stale catalog mode** — if the store access breaks, work on last saved catalog with clear stale warning; allow manual list edits.
...
- **One combined shopping list** — always **copyable**; **the store link** when working (never the only path).
...
- **Scope:** one store (the store, Alabino 92); thin **store catalog** boundary for future stores.
- **Tech stack:** Next.js, modern UI components, Supabase (data, auth, cloud functions).
- **Catalog:** unofficial access (e.g. `store-catalog-api`); sync + copyable list + stale-catalog mode (A4).
