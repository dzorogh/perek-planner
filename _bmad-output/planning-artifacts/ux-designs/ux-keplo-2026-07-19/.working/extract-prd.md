...
Personal web batch-cooking planner: open the app → pick 1–4 days → get Recipes for one store order and one cook, with portions by day and meal for two people by default. Purchase happens outside the app (copyable Shopping list always; store link when available).
...
- **External:** the store retailer site for cart edits and purchase completion
...
- Select specific store (v1 default context: д. Алабино, 92); concrete store, not free-text address
...
- Optional store link when available; must not be only purchase path
...
- **Scope:** single operator (Sergey); grocery chain only in v1 UI; Menu length 1–4 days capped by fridge-keep
