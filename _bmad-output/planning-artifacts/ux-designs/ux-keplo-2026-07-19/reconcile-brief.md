...
- **Batch meal-prep model** — one store order, one cook session, eat from the fridge until the batch is nearly finished (`extract-brief.md` one-liner; UJ-1 in memlog).
...
- **Shopping list handoff** — one combined list, **always copyable**, optional store link when working, never the only path (brief journeys 5, 7; stitch hard constraints).
...
- **Store context** — single grocery chain; **store selection surface** with default **д. Алабино, 92** (brief surfaces; memlog store selection decision).
...
| **Manual shopping-list edits in app** (especially under stale catalog) | Journey 10 stale mode; open question on sync | **Dropped as primary path.** No in-app list editing; cart edits happen on the store site (memlog override line 16; PRD non-goals). |
