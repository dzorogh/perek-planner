...
- **slot-cell** — Day × meal intersection. Contains one or more `slot-dish-line` rows from the meal template. Default fill `{colors.empty-slot}`; no always-visible action button row. Multi-line lunch/dinner cells grow vertically while keeping calm meal-lane density.
- **slot-dish-line** — Plate role in `{colors.slot-label}` / caption weight; dish name remains the hero (`{typography.slot-name}`) and opens **Recipe** text. v1 RU roles: Суп / Белок / Овощи / Углеводы / Основное / Перекус. Multi-role coverage uses the same quiet label treatment (e.g. «Белок · Углеводы»). Empty role: placeholder in `{colors.slot-label}` on the line.
- **slot-overflow** — Equal-weight actions behind a always-focusable ⋯ control (`Заменить`, `Заменить все`, `Никогда не предлагать`), scoped to a Plate role / dish line. Menu uses `{colors.surface}` + `{colors.border}` + soft indigo shadow. Do not pin these actions as visible buttons on every cell.
