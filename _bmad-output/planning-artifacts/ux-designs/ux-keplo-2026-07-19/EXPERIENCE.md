...
| **Menu** + slot edit | After generation / wizard step | Model C eat view as **meal lanes** (meal type × day). Each `slot-cell` shows role-labeled `slot-dish-line` rows (lunch: soup + Harvard; dinner: Harvard; breakfast/snack: same chrome, simpler templates). AI may repeat simple home dishes across days — no batch-component layer, no always-visible per-slot CTA row. Empty roles OK; **Refusal** / replace via `slot-overflow` per role; **Snack**s as a **Перекус** lane when planned → [mock](mockups/mock-menu-edit.html) |
...
| `meal-lane` / `slot-cell` | **Menu** + slot edit | Meal-type row with one `slot-cell` per day (model C — eat view only; no parallel batch-component surface). Each cell contains ordered `slot-dish-line` rows from the meal template (same chrome for breakfast, lunch, dinner, snack). AI may assign the same simple home dish across days — repetition is success, not a UI warning or “same as” badge. → [mock](mockups/mock-menu-edit.html) |
| `slot-dish-line` | **Menu** + slot edit | Quiet Russian **Plate role** label + dish name. Role readable without opening ⋯. v1 labels: Суп / Белок / Овощи / Углеводы / Основное / Перекус. Multi-role one-pots show coverage on the line (e.g. «Белок · Углеводы»). Name opens `recipe-text-panel`. |
| `slot-overflow` | **Menu** + slot edit | Equal-weight actions behind ⋯: «Заменить», «Заменить все», «Никогда не предлагать» (**Refusal**) — actions target a **Plate role** / dish line. No always-visible multi-button row on each cell. Trigger always keyboard-focusable (not hover-only). AI resuggest only — no manual History pick. **Recipe** name (not ⋯) opens `recipe-text-panel`. Empty roles/slots allowed. |
...
- **Plate role** — functional role of a Slot dish (`soup`, `protein`, `veg`, `carb`, `main`, `snack`, …)
- **Meal template** — code-defined ordered Plate roles for a meal type
- **Harvard plate** — lunch/dinner second course: protein + vegetables + carb (or multi-role Recipe coverage)
- **Slot dish** — Recipe/item under a Plate role inside a meal slot; uniform for all meal types
