---
title: "Brief ↔ PRD reconciliation: perek-planner"
status: draft
created: 2026-07-19
updated: 2026-07-19
inputs:
  - briefs/brief-perek-planner-2026-07-19/brief.md
  - briefs/brief-perek-planner-2026-07-19/addendum.md
  - prds/prd-perek-planner-2026-07-19/prd.md
  - prds/prd-perek-planner-2026-07-19/addendum.md
---
...
The PRD preserves the core job: single operator (Sergey), default two people, one cook + one order per Menu, Checked matches before cart, pantry opt-in, fridge-keep capping window length, copyable Shopping list with optional store link, Perekrestok-only v1 with a thin store adapter, stale-catalog mode, and success judged by real fridge cycles—not clever AI.
...
| Brief | PRD |
|---|---|
| One store: Perekrestok for address д. Алабино, 92 | FR-16: select a **specific Perekrestok store**; v1 chain Perekrestok only. Alabino appears in glossary as Product context, not as the only selectable store. |
...
| Brief | PRD |
|---|---|
| If Perekrestok access breaks: last saved catalog + stale warning + **allow manual list edits** | FR-18: stale catalog + warning. **Non-Goal:** “Editing the Shopping list or store cart inside this app.” Cart edits on store site (FR-17, §4.5). |
...
Brief Solution names Next.js / Supabase; brief addendum documents unofficial `perekrestok-api`, X5 B2B, VkusVill later. PRD correctly parks stack and store research in addendum—tone of “fragile unofficial pipe” is quieter in the PRD spine (only stale-catalog FR remains).
...
| Perekrestok / Alabino | Glossary, FR-16 | **Delta D5** (selectable store) |
...
**Input:** brief-perek-planner-2026-07-19
