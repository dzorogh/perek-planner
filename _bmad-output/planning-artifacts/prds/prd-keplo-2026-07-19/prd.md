...
PRD for a personal hobby product (single operator: Sergey). Downstream readers: UX, architecture, and implementation of keplo. Builds on Product Brief `briefs/brief-keplo-2026-07-19/`. Glossary-anchored vocabulary; features grouped with nested FRs; assumptions tagged inline.

## 1. Vision

A personal web batch-cooking planner: open the app → pick 1–4 days → get Recipes for one store order and one cook, with portions by day and meal for two people by default. Recipes enter the plan only when Critical ingredients are matched to real store Products; purchase happens outside the app (copyable Shopping list always; store link when available).
...
- **Product** — A store catalog item for the selected store (v1 default context: д. Алабино, 92).
...
- **Pantry item** — Spices, sauces, or staples that gate Recipe eligibility; appear on the Shopping list by default in v1 (Sergey filters at store order time).
...
**Description:** A Recipe enters an order only when every Critical ingredient has a Checked match to a Product. Pantry items gate eligibility and join the Shopping list by default (Sergey filters in the store). Fridge-keep must cover the Menu. Cheaper Product analogs are preferred at medium aggressiveness without collapsing quality. Realizes UJ-1.
...
Pantry items affect Recipe eligibility; in v1 they appear on the Shopping list by default so Sergey can filter them at store order time (no per-item in-app opt-in prompt).
...
**Description:** Sergey selects a specific store. The app syncs that store’s Product catalog. If a Recipe’s Critical ingredients have no Product (or suitable analog) in stock today, that Recipe is not suggested at all — no stock UI, no separate “fallback” flow. If store sync fails or the catalog is stale, Menu planning is blocked with a clear stale warning until a fresh sync succeeds. Cart edits happen on the store site, not in this app. Realizes UJ-1.

**Functional Requirements:**

#### FR-16: Select store

Sergey can select a specific store whose catalog drives Products and today’s stock.
...
**Description:** One Shopping list for the Menu. Always copyable. Optional the store link when available — never the only way to buy. Price and nutrition only when the catalog provides them. Sergey edits the cart on the store site, not in this app. Realizes UJ-1.
...
When a working the store link can be produced, Sergey can open it; it must not be the only purchase path.
