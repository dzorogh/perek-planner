...
- No public the store consumer checkout API.
- PyPI package `store-catalog-api`: unofficial reverse-engineering; plausible for personal catalog and prices; not reliable for ordering or guaranteed stock.
...
- First-version ordering posture: hand off a list (and a link when possible), not checkout inside keplo.
...
| Planning takes too long | User opens the store instead | Repeat last window; ready packs of checked recipes |
