...
sources:
  ...
  - ../planning-artifacts/briefs/brief-perek-planner-2026-07-19/brief.md
  - ../planning-artifacts/briefs/brief-perek-planner-2026-07-19/addendum.md
---
...
- **CAP-10**
  - **intent:** Pantry/staple items gate Recipe eligibility by catalog presence and appear on the Shopping list by default so the operator can filter them at the store.
...
- **CAP-13**
  - **intent:** Operator can select a concrete Perekrestok store in Settings that drives catalog and stock (default д. Алабино, 92).
  - **success:** A concrete store (not free-text address) is selected once in Settings and is not required before every new Menu; v1 UI is Perekrestok only.
...
- **CAP-18**
  - **intent:** When a working Perekrestok link can be produced, operator can open it as a secondary handoff.
...
- v1 grocery chain is Perekrestok only; keep a thin store-adapter boundary for a later chain.
...
- Unofficial Perekrestok catalog access is enough for personal v1 catalog/prices (not ordering or stock guarantees).
