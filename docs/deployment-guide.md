...
1. Apply all files in `supabase/migrations/` in timestamp order to the Supabase project (agents apply via Supabase MCP)  
...
4. **Auth → Providers / Password security:** enable **Leaked password protection** (HaveIBeenPwned). Advisors WARN until this is on.

## App Deploy Checklist

1. Set env vars on Dokploy (`KEPLO_ENV=production`, no auth bypass)  
2. `npm run build` succeeds  
3. Confirm login → create menu (needs OpenRouter) → UJ-1 → shopping list copy  
4. Confirm OpenRouter key is server-scoped  
5. Confirm leaked-password protection enabled in Supabase Auth  

## CI / CD

No committed GitHub Actions pipeline observed in-repo. Verification is script-driven (`npm run verify`). Optional Playwright: `npm run verify:e2e` (needs operator + OpenRouter).
...
- Bypass auth must remain production-inert (`NODE_ENV=production` or `KEPLO_ENV=production`)  
